...
/* variables for data gathering and transmit */
...
//float aiInData[AI_SVM_ACC_MOTION_IN_1_SIZE];
...
//float aiOutData[AI_SVM_ACC_MOTION_OUT_2_SIZE];
//AI_ALIGNED(4) ai_float aiOutData[AI_SVM_ACC_MOTION_OUT_2_SIZE];
...
const float data_min[NUM_AXES] = { -2896.0f, -1328.0f, 10224.0f };
const float data_max[NUM_AXES] = { 2960.0f, 2320.0f, 23040.0f };
...
void add_data_to_buffer(int16_t x, int16_t y, int16_t z) {
...
void write_buffer_to_file() {
	...
	sprintf(strTemp, "START_TRANSMISSION\r\n");
	CDC_Transmit_FS((uint8_t*) strTemp, strlen(strTemp));

	for (int i = 0; i < BUFFER_SIZE; i++) {
		sprintf(strTemp, "%d,%d,%d\r\n", buffer[i].x, buffer[i].y, buffer[i].z);
		CDC_Transmit_FS((uint8_t*) strTemp, strlen(strTemp));
		HAL_Delay(5);
	}

	sprintf(strTemp, "END_TRANSMISSION\r\n");
	CDC_Transmit_FS((uint8_t*) strTemp, strlen(strTemp));
...
//
//
////	return (model_output >= 0.0f) ? 1 : 0;
////	return 1;
//}

uint8_t make_prediction(ai_float model_output) {
    if (model_output == 0) {
        sprintf(strTemp, "No prediction (model output is zero)\r\n");
        CDC_Transmit_FS((uint8_t*) strTemp, strlen(strTemp));
        HAL_Delay(5);
        return 255;
    } else {
...
//			BSP_ACCELERO_GetXYZ(accDataxyz);
			...
			write_index += 3;
			if (write_index == AI_SVM_ACC_MOTION_IN_1_SIZE) {
				write_index = 0;

				sprintf(strPrediction, "input aiInData[0]: %.5f\r\n", aiInData[0]);
				CDC_Transmit_FS(strPrediction, strlen(strPrediction));
				HAL_Delay(5);
				...
				printf("Model output: ");
				for (uint32_t i = 0; i < AI_SVM_ACC_MOTION_OUT_1_SIZE; i++) {
					printf("%f ", aiOutData[i]);
				}
				printf("\r\n");

				sprintf(strTemp, "aiOutData{0}: %.5f\r\n", aiOutData[0]);
				CDC_Transmit_FS(strTemp, strlen(strTemp));
				HAL_Delay(5);


				sprintf(strTemp, "raw bytes aiOutData{0}: %02X\r\n", aiOutData[0]);
				CDC_Transmit_FS(strTemp, strlen(strTemp));
				HAL_Delay(5);

				ai_u8 predicted_class = make_prediction(aiOutData[0]);
				sprintf(strPrediction, "predicted_class: %d\r\n", predicted_class);
