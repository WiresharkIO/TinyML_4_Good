...
int8_t strTemp[100];
char strPrediction[50];
uint32_t timestamp;

//volatile uint32_t dataRdyIntReceived;

/* variables for inference */
float segmented_data[AI_ACCELEROMETER_MOTION_IN_1_HEIGHT][AI_ACCELEROMETER_MOTION_IN_1_CHANNEL];
int buf_idx = 0;

ai_handle accelerometer_motion;
float aiInData[AI_ACCELEROMETER_MOTION_IN_1_SIZE];
float aiOutData[AI_ACCELEROMETER_MOTION_OUT_1_SIZE];
ai_u8 activations[AI_ACCELEROMETER_MOTION_DATA_ACTIVATIONS_SIZE];
ai_buffer *ai_input;
ai_buffer *ai_output;
//const char *activities[AI_ACCELEROMETER_MOTION_OUT_1_SIZE] = { "Idle", "UpDown" };
